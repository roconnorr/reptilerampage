...
namespace BeautifyEffect
{
	...
	public class BeautifyEffectInspector : Editor
	{
		...
		const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint432";
		Beautify _effect;
		...
		public override void OnInspectorGUI ()
		{
			...
			if (EditorPrefs.GetInt (BEAUTIFY_BUILD_HINT) == 0) {
				...
				EditorUtility.DisplayDialog ("Beautify v4.3", "Before building your game, please check the Build Options section in Beautify's inspector for new shader capabilities and make sure you disable unused features to optimize build size and compilation time.\n\nOtherwise when you build the game it will take a long time during shader compilation!", "Ok");
			}
...
//			}
			...
			EditorGUILayout.EndHorizontal ();
			if (isFeatureEnabled (Beautify.SKW_VIGNETTING)) {
				...
				if (expandVignettingSection) {
					...
					if (isFeatureEnabled (Beautify.SKW_VIGNETTING_MASK)) {
...
//												GUILayout.Label (new GUIContent ("Frame", "Enables colored frame effect. Color alpha specifies intensity of effect."), labelStyle, GUILayout.Width (90));
			...
			EditorGUILayout.EndHorizontal ();
			if (isFeatureEnabled (Beautify.SKW_FRAME)) {
				...
				if (expandFrameSection) {
					...
					if (isFeatureEnabled (Beautify.SKW_FRAME_MASK)) {
