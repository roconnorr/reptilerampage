...
//								public static string SKW_TINT = "BEAUTIFY_TINT";
		...
		void OnRenderImage (RenderTexture source, RenderTexture destination)
		{
			...
			if (renderPass==0) {
				RenderTexture rtBeauty = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
				Graphics.Blit (source, rtBeauty, bMat, 1);
				bMat.SetTexture("_OverlayTex", rtBeauty);
	...
	#region Settings stuff
		...
		void UpdateMaterialProperties ()
		{
			...
			if (_quality == BEAUTIFY_QUALITY.BestQuality) {
				if (bMatDesktop == null) {
					bMatDesktop = new Material (Shader.Find ("Beautify/Beautify")); // Instantiate (Resources.Load<Material> ("Materials/Beautify"));
					bMatDesktop.hideFlags = HideFlags.DontSave;
				}
				bMat = bMatDesktop;
			} else {
				if (bMatMobile == null) {
					bMatMobile = new Material (Shader.Find ("Beautify/BeautifyMobile")); // Instantiate (Resources.Load<Material> ("Materials/BeautifyMobile"));
